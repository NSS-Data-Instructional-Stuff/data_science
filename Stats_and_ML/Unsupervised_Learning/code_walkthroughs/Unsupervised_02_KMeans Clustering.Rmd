...
district_projection = districts_pca$x %>% 
  as_tibble() %>% 
  select(PC1, PC2) %>% 
  add_column("cluster" = district_clusters$cluster) %>% 
  cbind(districts %>% select(system_name))
```

```{r}
ggplot() +
  geom_point(data = district_projection, aes(x = PC1, y = PC2, color = as.factor(cluster))) +
...
district_projection = district_projection %>% 
  mutate(cluster = district_clusters$cluster)
...
```
After looking at this plot, I am going to go with 5 clusters.
